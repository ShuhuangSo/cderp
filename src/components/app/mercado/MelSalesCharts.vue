      ...
      this.getRequest(url).then(resp => {
        if (resp) {
          ...
          for (let s of seriesSet) {
            ...
            let list = this.salesReports.filter(d => d.shop === s);
            // 匹配得到该条折线，对应的所有横轴的值组成的数组
            let _data = this.dateList.map((d, index) => {
              let num = list.find(item => item.calc_date === d);
          ...
          this.salesReports.forEach(item=>{
            ...
            this.total.tAmount += item.profit // 毛利
          })
          ...
          let option = {
            title: {
              text: '区间总销量 '+ this.total.tQty +' 单',
              subtext: '毛利润: ￥' + this.total.tAmount.toFixed(2),
              left: 'center'
            },
            tooltip: {
              trigger: 'axis'
